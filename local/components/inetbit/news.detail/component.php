...
$pagerParameters["LNG"] = $_COOKIE["LNG"];
...
if($arParams["SHOW_WORKFLOW"] || $this->startResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()),$bUSER_HAVE_ACCESS, $arNavigation, $pagerParameters)))
{
	...
	$arFilter["PROPERTY_LNG.CODE"]=$_COOKIE["LNG"];
...
if(isset($arResult["ID"]))
{
	...
	if (empty($arResult["PROPERTIES"]["CHILD_PAGE"]["VALUE"])==false)
	{
		$arChain = [];
		Site::get_chain_list($arResult["IBLOCK_ID"],$arResult["PROPERTIES"]["CHILD_PAGE"]["VALUE"],$arChain);
		krsort($arChain);
		foreach($arChain as $kChain=>$vChain)
		{
            $ipropValues = new Iblock\InheritedProperty\ElementValues($vChain["IBLOCK_ID"], $vChain["ID"]);
            $ar = $ipropValues->getValues();
			if (empty($ar["ELEMENT_PAGE_TITLE"])==false)
			{
				$APPLICATION->AddChainItem($ar["ELEMENT_PAGE_TITLE"], $vChain["CODE"]);	
			}
		}
